...
@section('title', $metaTags->title ?? 'Verify Register OTP')
            ...
            <div class="col-md-6 d-flex mb-md-0 mb-5 align-items-center justify-content-center p-4">
                <div class="card shadow rounded-4 w-100" style="max-width: 450px;">
                    <div class="card-body p-4">
                        ...
                        <p class="text-muted text-center mb-4">Let's verify your Email</p>
                        <p class="mb-4 text-muted text-center">We’ve sent a verification code to <span class="primary_color">{{ $email }}</span></p>

                        <form method="post" action="{{ route('verifyRegisterEmailOtp') }}" id="otpForm">
                            @csrf
                            ...
                            <div class="d-flex justify-content-center align-items-center mt-3">
                                <p class="me-2 mb-0">Didn’t receive the code?</p>
                                <form method="POST" action="{{ route('resendMailOtp') }}">
                                    @csrf
                                    <input type="hidden" name="email" value="{{ $email }}">
                                    <button type="submit" class="btn btn-link p-0 m-0 align-baseline primary_color text-decoration-none">
                                        Resend Code
                                    </button>
                                </form>
                            </div>
